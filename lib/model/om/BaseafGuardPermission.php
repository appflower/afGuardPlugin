...
/**
 * Base class that represents a row from the 'af_guard_permission' table.
 *
 *
 *
 * @package    propel.generator.plugins.afGuardPlugin.lib.model.om
 */
abstract class BaseafGuardPermission extends BaseObject implements Persistent
{
    /**
     * Peer class name
     */
    const PEER = 'afGuardPermissionPeer';

    /**
     * The Peer class.
     * Instance provides a convenient way of calling static methods on a class
     * that calling code may not be able to identify.
     * @var        afGuardPermissionPeer
     */
    ...
    /**
     * @var        PropelObjectCollection|afGuardGroupPermission[] Collection to store aggregation of afGuardGroupPermission objects.
     */
    protected $collafGuardGroupPermissions;
    protected $collafGuardGroupPermissionsPartial;
     ...
     * @return afGuardPermission The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[] = afGuardPermissionPeer::ID;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[] = afGuardPermissionPeer::NAME;
        }
    ...
    public function setDescription($v)
    {
        ...
        if ($this->description !== $v) {
            $this->description = $v;
            $this->modifiedColumns[] = afGuardPermissionPeer::DESCRIPTION;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 3; // 3 = afGuardPermissionPeer::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating afGuardPermission object", $e);
        }
    }
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($con === null) {
            $con = Propel::getConnection(afGuardPermissionPeer::DATABASE_NAME, Propel::CONNECTION_READ);
        }

        // We don't need to alter the object instance pool; we're just modifying this instance
        // already in the pool.

        $stmt = afGuardPermissionPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = afGuardPermissionQuery::create()
                ->filterByPrimaryKey($this->getPrimaryKey());
            $ret = $this->preDelete($con);
            // symfony_behaviors behavior
            foreach (sfMixer::getCallables('BaseafGuardPermission:delete:pre') as $callable)
            {
            ...
            if ($ret) {
                ...
                $this->postDelete($con);
                // symfony_behaviors behavior
                foreach (sfMixer::getCallables('BaseafGuardPermission:delete:post') as $callable)
                {
    ...
    public function save(PropelPDO $con = null)
    {
        ...
        try {
            ...
            // symfony_behaviors behavior
            foreach (sfMixer::getCallables('BaseafGuardPermission:save:pre') as $callable)
            {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                // symfony_behaviors behavior
                foreach (sfMixer::getCallables('BaseafGuardPermission:save:post') as $callable)
                {
                  call_user_func($callable, $this, $con, $affectedRows);
                }

                afGuardPermissionPeer::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->collafGuardGroupPermissions !== null) {
                foreach ($this->collafGuardGroupPermissions as $referrerFK) {
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        $this->modifiedColumns[] = afGuardPermissionPeer::ID;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . afGuardPermissionPeer::ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(afGuardPermissionPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(afGuardPermissionPeer::NAME)) {
            $modifiedColumns[':p' . $index++]  = '`name`';
        }
        if ($this->isColumnModified(afGuardPermissionPeer::DESCRIPTION)) {
            $modifiedColumns[':p' . $index++]  = '`description`';
        }

        $sql = sprintf(
            'INSERT INTO `af_guard_permission` (%s) VALUES (%s)',
    ...
    protected function doValidate($columns = null)
    {
        if (!$this->alreadyInValidation) {
            ...
            if (($retval = afGuardPermissionPeer::doValidate($this, $columns)) !== true) {
    ...
    {
        $pos = afGuardPermissionPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['afGuardPermission'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['afGuardPermission'][$this->getPrimaryKey()] = true;
        $keys = afGuardPermissionPeer::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
    ...
    {
        $criteria = new Criteria(afGuardPermissionPeer::DATABASE_NAME);

        if ($this->isColumnModified(afGuardPermissionPeer::ID)) $criteria->add(afGuardPermissionPeer::ID, $this->id);
        if ($this->isColumnModified(afGuardPermissionPeer::NAME)) $criteria->add(afGuardPermissionPeer::NAME, $this->name);
        if ($this->isColumnModified(afGuardPermissionPeer::DESCRIPTION)) $criteria->add(afGuardPermissionPeer::DESCRIPTION, $this->description);
    ...
    {
        $criteria = new Criteria(afGuardPermissionPeer::DATABASE_NAME);
        $criteria->add(afGuardPermissionPeer::ID, $this->id);
     ...
     * objects.
     *
     * @param object $copyObj An object of afGuardPermission (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getafGuardGroupPermissions() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addafGuardGroupPermission($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return afGuardPermission Clone of current object.
     * @throws PropelException
     */
     ...
     *
     * @return afGuardPermissionPeer
     */
    public function getPeer()
    {
        if (self::$peer === null) {
            self::$peer = new afGuardPermissionPeer();
        }
     ...
     *
     * @return afGuardPermission The current object (for fluent API support)
     * @see        addafGuardGroupPermissions()
     */
    public function clearafGuardGroupPermissions()
    {
        $this->collafGuardGroupPermissions = null; // important to set this to null since that means it is uninitialized
        $this->collafGuardGroupPermissionsPartial = null;

        return $this;
    }

    /**
     * reset is the collafGuardGroupPermissions collection loaded partially
     *
     * @return void
     */
    public function resetPartialafGuardGroupPermissions($v = true)
    {
        $this->collafGuardGroupPermissionsPartial = $v;
    }
     ...
     * If this afGuardPermission is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|afGuardGroupPermission[] List of afGuardGroupPermission objects
     * @throws PropelException
     */
    public function getafGuardGroupPermissions($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collafGuardGroupPermissionsPartial && !$this->isNew();
        if (null === $this->collafGuardGroupPermissions || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collafGuardGroupPermissions) {
                ...
                $collafGuardGroupPermissions = afGuardGroupPermissionQuery::create(null, $criteria)
                    ->filterByafGuardPermission($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collafGuardGroupPermissionsPartial && count($collafGuardGroupPermissions)) {
                      $this->initafGuardGroupPermissions(false);

                      foreach($collafGuardGroupPermissions as $obj) {
                        if (false == $this->collafGuardGroupPermissions->contains($obj)) {
                          $this->collafGuardGroupPermissions->append($obj);
                        }
                      }

                      $this->collafGuardGroupPermissionsPartial = true;
                    }

                    $collafGuardGroupPermissions->getInternalIterator()->rewind();
                    return $collafGuardGroupPermissions;
                }

                if($partial && $this->collafGuardGroupPermissions) {
                    foreach($this->collafGuardGroupPermissions as $obj) {
                        if($obj->isNew()) {
                            $collafGuardGroupPermissions[] = $obj;
                        }
                    }
                }

                $this->collafGuardGroupPermissions = $collafGuardGroupPermissions;
                $this->collafGuardGroupPermissionsPartial = false;
            }
        }
    ...
    {
        $afGuardGroupPermissionsToDelete = $this->getafGuardGroupPermissions(new Criteria(), $con)->diff($afGuardGroupPermissions);

        $this->afGuardGroupPermissionsScheduledForDeletion = unserialize(serialize($afGuardGroupPermissionsToDelete));

        foreach ($afGuardGroupPermissionsToDelete as $afGuardGroupPermissionRemoved) {
            $afGuardGroupPermissionRemoved->setafGuardPermission(null);
        }
     ...
     * @param PropelPDO $con
     * @return int             Count of related afGuardGroupPermission objects.
     * @throws PropelException
     */
    public function countafGuardGroupPermissions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        ...
        if (null === $this->collafGuardGroupPermissions || null !== $criteria || $partial) {
            ...
            if($partial && !$criteria) {
                return count($this->getafGuardGroupPermissions());
            }
    ...
    /**
     * Method called to associate a afGuardGroupPermission object to this object
    ...
    public function addafGuardGroupPermission(afGuardGroupPermission $l)
    {
        ...
        if (!in_array($l, $this->collafGuardGroupPermissions->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
    ...
    protected function doAddafGuardGroupPermission($afGuardGroupPermission)
    {
        ...
        $afGuardGroupPermission->setafGuardPermission($this);
    }

    /**
     * @param	afGuardGroupPermission $afGuardGroupPermission The afGuardGroupPermission object to remove.
     * @return afGuardPermission The current object (for fluent API support)
     */
    public function removeafGuardGroupPermission($afGuardGroupPermission)
    {
        if ($this->getafGuardGroupPermissions()->contains($afGuardGroupPermission)) {
            $this->collafGuardGroupPermissions->remove($this->collafGuardGroupPermissions->search($afGuardGroupPermission));
            if (null === $this->afGuardGroupPermissionsScheduledForDeletion) {
                $this->afGuardGroupPermissionsScheduledForDeletion = clone $this->collafGuardGroupPermissions;
                $this->afGuardGroupPermissionsScheduledForDeletion->clear();
            }
            $this->afGuardGroupPermissionsScheduledForDeletion[]= clone $afGuardGroupPermission;
            $afGuardGroupPermission->setafGuardPermission(null);
        }
     ...
     * Otherwise if this afGuardPermission is new, it will return
     * an empty collection; or if this afGuardPermission has previously
     * been saved, it will retrieve related afGuardGroupPermissions from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in afGuardPermission.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     ...
     * @return PropelObjectCollection|afGuardGroupPermission[] List of afGuardGroupPermission objects
     */
    public function getafGuardGroupPermissionsJoinafGuardGroup($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = afGuardGroupPermissionQuery::create(null, $criteria);
    ...
    public function setafGuardUserPermissions(PropelCollection $afGuardUserPermissions, PropelPDO $con = null)
    {
        ...
        foreach ($afGuardUserPermissionsToDelete as $afGuardUserPermissionRemoved) {
            $afGuardUserPermissionRemoved->setafGuardPermission(null);
        }
    ...
    protected function doAddafGuardUserPermission($afGuardUserPermission)
    {
        ...
        $afGuardUserPermission->setafGuardPermission($this);
    }
    ...
    public function removeafGuardUserPermission($afGuardUserPermission)
    {
        if ($this->getafGuardUserPermissions()->contains($afGuardUserPermission)) {
            ...
            $afGuardUserPermission->setafGuardPermission(null);
        }
     ...
     * @return PropelObjectCollection|afGuardUserPermission[] List of afGuardUserPermission objects
     */
    public function getafGuardUserPermissionsJoinafGuardUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = afGuardUserPermissionQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(afGuardPermissionPeer::DEFAULT_STRING_FORMAT);
    }
    ...
    public function __call($name, $params)
    {
        ...
        // symfony_behaviors behavior
        if ($callable = sfMixer::getCallable('BaseafGuardPermission:' . $name))
        {
