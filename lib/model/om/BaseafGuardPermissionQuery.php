...
abstract class BaseafGuardPermissionQuery extends ModelCriteria
{
     ...
     * @param   afGuardPermissionQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     *
     * @return   afGuardPermission|afGuardPermission[]|mixed the result, formatted by the current formatter
     */
    public function findPk($key, $con = null)
    {
        ...
        if ((null !== ($obj = afGuardPermissionPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
     ...
     *
     * @return                 afGuardPermission A model object, or null if the key is not found
    ...
    {
        $sql = 'SELECT `id`, `name`, `description` FROM `af_guard_permission` WHERE `id` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new afGuardPermission();
            $obj->hydrate($row);
            afGuardPermissionPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|afGuardPermission[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(afGuardPermissionPeer::ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(afGuardPermissionPeer::ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
