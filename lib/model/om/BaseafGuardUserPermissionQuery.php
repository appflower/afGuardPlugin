...
abstract class BaseafGuardUserPermissionQuery extends ModelCriteria
{
     ...
     * @param   afGuardUserPermissionQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     *
     * @return   afGuardUserPermission|afGuardUserPermission[]|mixed the result, formatted by the current formatter
     */
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            afGuardUserPermissionPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|afGuardUserPermission[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(afGuardUserPermissionPeer::USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(afGuardUserPermissionPeer::USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByPermissionId($permissionId = null, $comparison = null)
    {
        if (is_array($permissionId)) {
            ...
            if (isset($permissionId['min'])) {
                $this->addUsingAlias(afGuardUserPermissionPeer::PERMISSION_ID, $permissionId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($permissionId['max'])) {
                $this->addUsingAlias(afGuardUserPermissionPeer::PERMISSION_ID, $permissionId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
     ...
     *
     * @param   afGuardUser|PropelObjectCollection $afGuardUser The related object(s) to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return                 afGuardUserPermissionQuery The current query, for fluid interface
    ...
    public function filterByafGuardUser($afGuardUser, $comparison = null)
    {
        ...
        } elseif ($afGuardUser instanceof PropelObjectCollection) {
            if (null === $comparison) {
                $comparison = Criteria::IN;
            }
