...
abstract class BaseafGuardUserGroupQuery extends ModelCriteria
{
     ...
     * @param   afGuardUserGroupQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     *
     * @return   afGuardUserGroup|afGuardUserGroup[]|mixed the result, formatted by the current formatter
     */
    ...
    protected function findPkSimple($key, $con)
    {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            afGuardUserGroupPeer::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1])));
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|afGuardUserGroup[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(afGuardUserGroupPeer::USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(afGuardUserGroupPeer::USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
    ...
    public function filterByGroupId($groupId = null, $comparison = null)
    {
        if (is_array($groupId)) {
            ...
            if (isset($groupId['min'])) {
                $this->addUsingAlias(afGuardUserGroupPeer::GROUP_ID, $groupId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($groupId['max'])) {
                $this->addUsingAlias(afGuardUserGroupPeer::GROUP_ID, $groupId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
