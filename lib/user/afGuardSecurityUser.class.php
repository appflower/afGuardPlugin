<?php
class afGuardSecurityUser extends sfBasicSecurityUser
{
  protected
    $user = null;
  ...
  public function initialize(sfEventDispatcher $dispatcher, sfStorage $storage, $options = array())
  {
    ...
    if (!$this->isAuthenticated())
    {
      ...
      $this->getAttributeHolder()->removeNamespace('afGuardSecurityUser');
      $this->user = null;
    }
  }
  ...
  {
    // signin
    $this->setAttribute('user_id', $user->getId(), 'afGuardSecurityUser');
    ...
    if ($remember)
    {
      // remove old keys
      $c = new Criteria();
      $expiration_age = sfConfig::get('app_af_guard_plugin_remember_key_expiration_age', 15 * 24 * 3600);
      $c->add(afGuardRememberKeyPeer::CREATED_AT, time() - $expiration_age, Criteria::LESS_THAN);
      afGuardRememberKeyPeer::doDelete($c);

      // remove other keys from this user
      $c = new Criteria();
      $c->add(afGuardRememberKeyPeer::USER_ID, $user->getId());
      afGuardRememberKeyPeer::doDelete($c);
      ...
      // save key
      $rk = new afGuardRememberKey();
      $rk->setRememberKey($key);
      $rk->setAfGuardUser($user);
      $rk->setIpAddress($_SERVER['REMOTE_ADDR']);
      $rk->save($con);

      // make key as a cookie
      $remember_cookie = sfConfig::get('app_af_guard_plugin_remember_cookie_name', 'sfRemember');
  ...
  {
    if (!$this->user && $id = $this->getAttribute('user_id', null, 'afGuardSecurityUser'))
    {
      $this->user = afGuardUserPeer::retrieveByPk($id);
